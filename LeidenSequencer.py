...
def LeidenSequencer( object ):
    ...
    def Book( self ):
        ...
        sys.OutputFile = open( Output, "w")
        sys.LogFile = open( Log, "w")
        sys.Sysout.append( LogFile )
    ...
    def PrintConfig( self ):
        ...
        for f in self.Sysout:
            ...
            print( "# Temperature is recorded either after %d minutes or when the rate of change is less than %.2f mK/min" % (self.Timeout, self.dTdt) , file=f )
    ...
    def Sequence( self ):
        ...
        for setpoint in self.Setpoints:
            ...
            self.T1['ts'] = TimeSince( StartTimeOffset )
            self.T1['pw'] = power
            for c in self.Channels:
                for f in self.Sysout:
                    print( '# Setting scanner channel to ', c)
                self.controller.SetChannel( c )
                self.controller.wait(5)
                T1[c] = self.controller.ReadKelvin( c )
            ...
            while True:
                ...
                # If not stabilized yet, wait for the specified interval and then update T1
                    ...
                    time.sleep( self.Interval )
                    self.UpdateTemperature( T1, T2 )

            self.T1['ts'] = TimeSince( self.StartTimeOffset )
            self.ReadTemperature( T1)
    ...
    def CheckRate( self, Channel, T2, T1 ):
