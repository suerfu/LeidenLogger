...
class MaxiGauge (object):
    ...
    def getACQorNAK(self):
        ...
        if len(returncode)<3: 
            self.debugMessage('Only received a line termination from MaxiGauge. Was expecting ACQ or NAK.')
        ...
        if len(returncode)>2 and returncode[-3] != C['ACQ']: 
            self.debugMessage('Expecting ACQ or NAK from MaxiGauge but neither were sent.')
        # if no exception raised so far, the interface is just fine:
        return returncode[:-(len(LINE_TERMINATION))]
